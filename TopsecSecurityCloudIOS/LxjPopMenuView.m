...
@implementation LxjPopMenuView

+ (instancetype)menuView
{
    LxjPopMenuView *view = [[LxjPopMenuView alloc]initWithFrame:[UIScreen mainScreen].bounds];
    return view;
}
...
- (void)setupSubviews
{
    ...
    if (count == 2 || count == 4) {
        ...
        firstItemY = yStart;
        NSInteger index = 0;
        for (LxjPopMenuitem *item in self.menuItems) {
...
- (void)disappearMenuItem:(LxjPopMenuitem *)item animated:(BOOL )animted
{
...
- (void)appearMenuItem:(LxjPopMenuitem *)item animated:(BOOL )animated
{
