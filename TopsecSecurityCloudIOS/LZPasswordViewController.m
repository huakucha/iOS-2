...
- (instancetype)initWithState:(LZNumberPasswordState)state {
    ...
    if (self) {
        ...
        NSString * listtext = [ipset objectForKey:@"numberPasswordValueKey"];
        
        
        
        if (listtext.length>0 && state==0) {
            _passwordStete = LZNumberPasswordStateReset;
        }else{
...
- (void)setupNavBar {
    ...
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, PLZSCREEN_WIDTH, 44)];
    bgView.backgroundColor = [UIColor orangeColor];
    [self.view addSubview:bgView];
    
    UIButton *leftButton = [UIButton buttonWithType:UIButtonTypeCustom];
    leftButton.frame = CGRectMake(10, 11, 60, 22);
    ...
    label.center = CGPointMake(PLZSCREEN_WIDTH/2.0, 22);
    label.bounds = CGRectMake(0, 0, 100, 22);
    label.text = @"设置密码";
